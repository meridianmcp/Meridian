...
@pytest.mark.asyncio
async def test_proposal_schema_supports_idempotency_key(db):
    """867317f6 — add_workspace_proposal's idempotency_key column exists on
    every backend (see _migrate_workspace_proposals /
...
@pytest.mark.asyncio
async def test_postgres_proposal_migration_adds_idempotency_key_and_unique_index():
    """867317f6 — the Postgres upgrade path must also self-heal onto the new
    idempotency_key column + its tenant-scoped partial unique index, mirroring
    the SQLite guarded migration."""
    ...
    assert "idx_workspace_proposals_idempotency" in conn.script
    assert "COALESCE(tenant_id, '')" in conn.script
