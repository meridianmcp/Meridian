...
# ---------------------------------------------------------------------------
# 7. 34f76536 -- settings.json portability regression guard
# ---------------------------------------------------------------------------

# The 2026-08-07 bug (fe83c74b): a single developer's absolute Windows
# profile path checked directly into the shared .claude/settings.json,
# outside the $CLAUDE_PROJECT_DIR substitution -- broke Stop/SessionStart
# hooks for every OTHER machine/user that checked out this repo. Ban any
# hardcoded per-user home-directory segment in a non-$CLAUDE_PROJECT_DIR hook
# command going forward.
_PERSONAL_PATH_RE = re.compile(r"C:\\Users\\[^\\\"]+|/home/[^/\"]+|/Users/[^/\"]+")


def test_real_repo_settings_json_no_hardcoded_personal_paths():
    """No hook `command` string in the shared .claude/settings.json may embed
    a hardcoded per-user path segment (C:\\Users\\<name>, /home/<name>,
    /Users/<name>) unless it is $CLAUDE_PROJECT_DIR-scoped. A genuinely
    per-machine global hook (e.g. meridian-start/meridian-stop, written by
    hooks.ps1/hooks.sh) belongs in the user's own GLOBAL ~/.claude/settings.json,
    never in this repo-tracked file -- that duplication is exactly what
    fe83c74b got wrong."""
    settings_path = _REPO_ROOT / ".claude" / "settings.json"
    if not settings_path.exists():
        pytest.skip("no .claude/settings.json in this checkout")
    ...
    offenders = [
        (event, command)
        for event, command in hook_paths.parse_hook_commands(settings)
        if not hook_paths.is_project_relative_command(command)
        and _PERSONAL_PATH_RE.search(command)
    ]
    assert not offenders, (
        f"hardcoded per-user path(s) checked into .claude/settings.json "
        f"(belongs in the global ~/.claude/settings.json instead): {offenders}"
    )


# ---------------------------------------------------------------------------
# 8. 34f76536 -- GET /hooks/diagnostics: diagnose_configured_hooks wired live
# ---------------------------------------------------------------------------


def test_hooks_diagnostics_route_reports_advisory_not_hard_error(client, tmp_path, monkeypatch):
    """A missing OPTIONAL global hook (e.g. meridian-stop.ps1 before hooks.ps1
    has run) must surface as an advisory field in the response, never as an
    HTTP error -- and a present REQUIRED hook resolves ok alongside it."""
    repo = tmp_path / "repo"
    hooks_dir = repo / ".claude" / "hooks"
    hooks_dir.mkdir(parents=True)
    (hooks_dir / "present.ps1").write_text("# ok", encoding="utf-8")
    settings = {
        "hooks": {
            "Stop": [
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "shell": "powershell",
                            "command": '& "$CLAUDE_PROJECT_DIR\\.claude\\hooks\\present.ps1"',
                        }
                    ],
                },
                {
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "shell": "powershell",
                            "command": '& "C:\\Users\\someone\\.claude\\hooks\\meridian-stop.ps1"',
                        }
                    ],
                },
            ],
    ...
    r = client.get("/hooks/diagnostics")
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["settings_found"] is True
    assert len(body["hooks"]) == 2

    present = next(d for d in body["hooks"] if "present.ps1" in (d["script_token"] or ""))
    missing_global = next(d for d in body["hooks"] if "meridian-stop" in (d["script_token"] or ""))
    assert present["status"] == hook_paths.STATUS_OK
    assert present["required"] is True
    assert missing_global["status"] == hook_paths.STATUS_OPTIONAL_ABSENT
    assert missing_global["required"] is False
    assert body["missing_required_count"] == 0


def test_hooks_diagnostics_route_flags_missing_required_hook(client, tmp_path, monkeypatch):
    """A genuinely missing $CLAUDE_PROJECT_DIR-scoped hook script is the real
    problem case -- it must be counted in missing_required_count, distinct
    from an expected-absent optional global hook."""
    repo = tmp_path / "repo"
                    ...
                    "hooks": [
                        {
                            "type": "command",
                            "shell": "powershell",
                            "command": '& "$CLAUDE_PROJECT_DIR\\.claude\\hooks\\does_not_exist.ps1"',
                        }
                    ],
                },
            ],
    ...
    assert body["hooks"][0]["status"] == hook_paths.STATUS_MISSING_REQUIRED
    assert body["hooks"][0]["required"] is True


def test_hooks_diagnostics_route_no_settings_file_is_advisory_not_error(client, tmp_path, monkeypatch):
    """No .claude/settings.json at all (e.g. a bare checkout) must be a
    structured, advisory `settings_found: false` -- never a 404/500 -- since
    this endpoint is meant to be safe to poll unconditionally."""
    empty_repo = tmp_path / "no_settings_repo"
    empty_repo.mkdir()

    monkeypatch.setattr(hook_paths, "resolve_active_repo_root", lambda **kwargs: empty_repo)
