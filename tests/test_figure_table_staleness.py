...
def test_mcp_audit_figure_table_provenance_writes_durable_receipts(tmp_path, monkeypatch):
    """W31-A -- every figure/table this tool resolves gets a durable,
    retrievable artifact_provenance_receipt row, not just a transient
    report dict that vanishes once the call returns."""
    async def _run():
        ...
        db = await db_module.init_db(":memory:")
        try:
            proj = await db_module.create_project(db, "audit-proj-receipts")
            pid = proj["id"]
            outputs_dir = tmp_path / "outputs"
            outputs_dir.mkdir()
            (outputs_dir / "loss_curve.csv").write_text(
                "# generated by train.py\nepoch,loss\n1,0.5\n"
            )
            asset = outputs_dir / "loss_curve.csv"
            ...
            fig_res = await mh._dispatch_mcp_tool(
                "index_figure",
                {"project_id": pid, "doc": "thesis.docx",
                 "file_path": str(asset), "caption": "Figure 1: Loss curve"},
                db, str(tmp_path),
            )
            fig_id = fig_res["figure"]["id"]
            ...
            assert "error" not in res
            assert res["figures"][0]["status"] == "ok"
            assert res["receipts_recorded"] == 1

            receipt = await apr.find_recent_artifact_provenance_receipt(
                db, project_id=pid, artifact_id=f"figure:{fig_id}",
            )
            assert receipt is not None
            detail = apr._receipt_detail(receipt)
            assert detail["status"] == apr.RESOLVED  # audit "ok" -> binding "resolved"
            assert detail["generating_script"] == "train.py"
            assert detail["document_id"] == res["document_id"]

            by_doc = await apr.find_recent_artifact_provenance_receipts_for_document(
                db, project_id=pid, document_id=res["document_id"],
            )
            assert len(by_doc) == 1
        finally:
            await db.close()
...
def test_mcp_audit_figure_table_provenance_orphan_still_writes_a_receipt(tmp_path, monkeypatch):
    """A non-resolved (orphan) figure still gets a durable receipt --
    receipts record what was found, not only clean resolutions."""
    async def _run():
        from meridian import artifact_provenance_receipt as apr
        from meridian import server as mh
        sidecar = _seed_store_via_env(tmp_path, monkeypatch)
        db = await db_module.init_db(":memory:")
        try:
            proj = await db_module.create_project(db, "audit-proj-orphan-receipts")
            pid = proj["id"]
            seed = await doc_store.open_doc_store_for(
                plan=None, hosted=False, data_dir=str(tmp_path),
                tenant_pg_url=None, override_url=sidecar,
            ...
            fig_res = await mh._dispatch_mcp_tool(
                "index_figure",
                {"project_id": pid, "doc": "thesis.docx",
                 "caption": "Figure 1: Hand-drawn, no source file"},
                db, str(tmp_path),
            )
            fig_id = fig_res["figure"]["id"]
            ...
            assert res["figures"][0]["status"] == "orphan"

            receipt = await apr.find_recent_artifact_provenance_receipt(
                db, project_id=pid, artifact_id=f"figure:{fig_id}",
            )
            assert receipt is not None
            assert apr.receipt_status(receipt) == apr.ORPHANED
        finally:
            await db.close()
            await doc_store.close_all_doc_stores()
