...
# ---------------------------------------------------------------------------
# W31-A -- generating_script (the GENERATOR half of media-to-output-to-
# generator provenance) must survive _bind_one_artifact's resolved binding,
# not be silently computed and discarded.
# ---------------------------------------------------------------------------

class TestGeneratingScriptSurvivesBinding:
    @duckdb_required
    def test_exact_match_resolved_carries_generating_script(
        self, tmp_path: Path,
    ) -> None:
        f = tmp_path / "loss_curve.csv"
        f.write_text("# generated by train.py\nepoch,loss\n1,0.5\n")

        exact = PV.resolve_figure_output(str(tmp_path), str(f))
        assert exact is not None
        assert exact["generating_script"] == "train.py"  # sanity on the premise
        ...
        assert binding["status"] == PV.RESOLVED
        assert binding["match_type"] == "exact"
        assert binding["generating_script"] == "train.py"

    @duckdb_required
    def test_hash_mismatch_still_carries_generating_script(
        self, tmp_path: Path,
    ) -> None:
        # A same-size sibling forces the outputs walker's size-prefilter to
        # actually compute a hash (see test_exact_match_mismatched_expected_
        # hash_is_hash_mismatch above for the same technique) -- otherwise
        # this artifact's unique size would skip hashing entirely and land
        # on UNRESOLVED ("no hash to compare"), not the HASH_MISMATCH case
        # this test means to exercise.
        f = tmp_path / "loss_curve.csv"
        f.write_text("# generated by train.py\nepoch,loss\n1,0.5\n")
        sibling = tmp_path / "sibling_same_size.csv"
        sibling.write_text("# generated by other.py\nepoch,loss\n1,0.9\n")
        assert os.path.getsize(f) == os.path.getsize(sibling)  # sanity
    ...
    @duckdb_required
    def test_basename_match_carries_generating_script_as_diagnostic(
        self, tmp_path: Path,
    ) -> None:
        """Relocation-tolerant tier: generating_script rides along as the
        same non-authoritative diagnostic evidence as the rest of this tier
        -- present, but never proof of exact media-to-output lineage."""
        real_dir = tmp_path / "outputs_run_17"
        real_dir.mkdir()
        (real_dir / "metrics.csv").write_text(
            "# generated by sweep_runner.py\nepoch,loss\n1,0.5\n"
        )
        relocated_path = str(tmp_path / "docs_media" / "metrics.csv")
        ...
        assert binding["status"] == PV.UNRESOLVED
        assert binding["match_type"] == "basename"
        assert binding["generating_script"] == "sweep_runner.py"

    def test_orphaned_and_no_canonical_path_bindings_carry_none(
        self, tmp_path: Path,
    ) -> None:
        """Schema consistency: generating_script is always present on a
        binding (never a KeyError for a caller that reads it unconditionally),
        and is honestly None rather than fabricated when nothing resolved."""
        result = PV.bind_artifact_provenance(
            str(tmp_path),
            [
                {"artifact_id": "no-path", "kind": "figure", "canonical_path": None},
                {
                    "artifact_id": "unresolvable", "kind": "figure",
                    "canonical_path": str(tmp_path / "never_existed.png"),
                },
            ],
        )
        for binding in result["bindings"]:
            assert "generating_script" in binding
            assert binding["generating_script"] is None
