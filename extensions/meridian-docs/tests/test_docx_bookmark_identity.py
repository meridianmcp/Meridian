...
def _default_render_capability(monkeypatch):
    """016015e1/ddd79188 -- insert_caption / insert_highlighted_note
    (mode="inline") now invoke the real render-capability gate
    (render_gate.check_render_capability) AFTER structural verification
    passes. Tests in this file exercise STRUCTURAL correctness (bookmark
    identity) and must not depend on -- or be slowed/blocked by -- whichever
    render backends (LibreOffice, Word COM) happen to be installed on the
    machine running the suite. Stub a successful 'rendered' result by
    default, mirroring test_19be1551_insert_figure_block.py's fixture of the
    same name.
    """
    monkeypatch.setattr(
