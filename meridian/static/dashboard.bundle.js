(() => {
  ...
  try {
    Object.assign(window, { getPanelState: getPanelState2, toast: toast2, escapeHtml: escapeHtml2, formatRelativeTime: formatRelativeTime2, sessionAgeMs: sessionAgeMs2, isLiveSession: isLiveSession2, _colorForHuman: _colorForHuman2, _PLAN_LABELS: _PLAN_LABELS2, QUEUE_DONE_PAGE_SIZE: QUEUE_DONE_PAGE_SIZE2, SESSION_LIVE_WINDOW_MS, _HUMAN_COLORS });
  } catch (e) {
  }
  ...
  function showConnectDbModal() {
    ...
    box.querySelector("#connect-db-save").onclick = async () => {
      ...
      statusEl.style.color = "var(--muted)";
      try {
        await api2("/workspace/connect-db", { method: "POST", body: JSON.stringify({ url }) });
  ...
  var STORAGE_KEY = (k) => (isDemoMode() ? "meridian_demo_" : "meridian_") + k.replace(/^meridian[._]/, "");
  var NORTH_STAR_MIN_HEIGHT_PX = 180;
  var DEFAULT_MAX_PINNED_DECISIONS = 20;
  var DEFAULT_CONTEXT_THRESHOLD = 40;
  ...
  async function saveProjectSettings(projectId, patch) {
    const panel = getPanelState(projectId);
    const settings = await api2(`/projects/${projectId}/settings`, {
      method: "PATCH",
  ...
  function _demoTourDone2() {
    try {
      return localStorage.getItem(STORAGE_KEY("tour.done")) === "1";
    } catch (e) {
      return false;
    }
  }
  function _demoTourSavedStep2() {
    try {
      return parseInt(localStorage.getItem(STORAGE_KEY("tour.step")) || "0", 10) || 0;
    } catch (e) {
      return 0;
    }
  }
  function _demoTourSaveStep(step) {
    try {
      localStorage.setItem(STORAGE_KEY("tour.step"), String(step));
    } catch (e) {
    }
  }
  ...
  async function loadServerConfig() {
    ...
    if (window.location.pathname.startsWith("/demo")) {
      try {
        localStorage.removeItem(STORAGE_KEY(TABS_KEY));
      } catch (e) {
      }
      try {
        localStorage.removeItem(STORAGE_KEY(ACTIVE_PROJECT_KEY));
      } catch (e) {
      }
  ...
  function _armAccountSwitchWatch(loadedEmail) {
    ...
    setInterval(_checkAccountSwitch, 6e4);
  }
        ...
        displayConns.forEach((c) => {
          ...
          if (c.name && c.name !== "local" && adminFull) {
            ...
            del.onclick = async (e2) => {
              ...
              try {
                await api2("/config/connections/" + encodeURIComponent(c.name), { method: "DELETE" });
                popup.remove();
          ...
          item.onclick = async (e2) => {
            ...
            if (c.active) return;
            try {
              await api2("/config/connections", { method: "POST", body: JSON.stringify({ name: c.name, activate: true }) });
              await loadServerConfig();
              if (isHostedMode()) {
  ...
  async function checkGitStatus() {
    ...
    try {
      const s = await api2("/admin/git-status");
...
Current: ${current || "(none)"}`,
    ...
    try {
      const updated = await api2(`/projects/${t.id}/icon`, {
        method: "PATCH",
  ...
  async function _renameProject(t) {
    ...
    try {
      const updated = await api2(`/projects/${t.id}/rename`, {
        method: "POST",
  ...
  function activateTab(id) {
    ...
    try {
      localStorage.setItem(STORAGE_KEY(ACTIVE_PROJECT_KEY), id);
    } catch (e) {
    }
    const switcher = document.getElementById("project-switcher");
  ...
  `;
    ...
    if (decFormAdd) {
      const doAddDecision = async () => {
        ...
        try {
          await api2(`/projects/${project.id}/decisions-pinned`, { method: "POST", body: JSON.stringify({ title, body: body2, category }) });
    ...
    }
    {
      ...
      if (appendBtn && appendText) {
        appendBtn.onclick = async () => {
          ...
          try {
            await api2(`/projects/${project.id}/devlog`, { method: "POST", body: JSON.stringify({ text }) });
            appendText.value = "";
    ...
    if (!panel.liveWired) {
      ...
      if (addSubmit) {
        addSubmit.onclick = async () => {
          ...
          try {
            const sessions = await api2(`/projects/${pid}/sessions?active_only=true`);
            const activeSid = sessions && sessions[0] && sessions[0].id;
            await api2(`/projects/${pid}/hitl-requests`, {
              method: "POST",
              body: JSON.stringify({
        ...
        newSprintBtn.onclick = () => {
          ...
          const submit = async () => {
            ...
            try {
              ...
              await api2(`/projects/${projectId}/goal/sprint`, { method: "POST", body: JSON.stringify({ sprint: name }) });
  ...
  async function sprintItemEdit(projectId, itemId) {
    ...
    const save = async () => {
      ...
      try {
        await api2(`/projects/${projectId}/sprint-items/${itemId}`, {
          method: "PATCH",
  ...
  async function addSprintItemFromInput2(projectId) {
    ...
    try {
      await api2(`/projects/${projectId}/sprint-items`, {
        method: "POST",
  ...
  async function addLiveTask(projectId, description) {
    ...
    if (!sessionId) {
      try {
        const sessions = await api2(`/projects/${projectId}/sessions`);
...
get_context_block(project_id="${PROJECT_QUOTE}", mode="full")`;
  ...
  async function loadTimeline2(projectId) {
    ...
    let data;
    try {
      data = await api2(`/projects/${projectId}/timeline`);
    } catch (e) {
...
stop = ${JSON.stringify(stop)}`;
      ...
      const [notifResult, mcpResult, settingsResult, ntfyResult, ghResult] = await Promise.allSettled([
        api2("/settings/notifications"),
        api2("/settings/mcp-config"),
        loadProjectSettings(projectId),
        api2(`/projects/${projectId}/ntfy`),
        api2(`/projects/${projectId}/github/status`)
      ]);
        ...
        setTimeout(() => {
          ...
          if (genBtn) {
            genBtn.onclick = async () => {
              ...
              try {
                const tok = await api2("/auth/tokens", { method: "POST", body: JSON.stringify({ label: "mcp-config" }) });
...
project_id = "${displayPid}"`;
        ...
        setTimeout(() => {
          ...
          if (pushBtn) {
            pushBtn.onclick = async () => {
              ...
              try {
                await api2(`/projects/${projectId}/github/push-mcp-template`, { method: "POST" });
    ...
    <div style="margin-top:8px;display:flex;gap:8px;align-items:center">
      ...
      setTimeout(() => {
        ...
        (async () => {
          try {
            const s = await api2("/workspace/settings");
        ...
        async function renderWsDecisions() {
          if (!decList) return;
          try {
            const items = await api2("/workspace/decisions");
            ...
            decList.querySelectorAll("button[data-did]").forEach((btn) => {
              btn.onclick = async () => {
                ...
                try {
                  await api2(`/workspace/decisions/${btn.dataset.did}`, { method: "DELETE" });
                  renderWsDecisions();
                } catch (e) {
                  alert("Error: " + e);
                }
        ...
        if (decAdd) decAdd.onclick = async () => {
          ...
          try {
            await api2("/workspace/decisions", { method: "POST", body: JSON.stringify({ title, body: body2 }) });
          ...
          try {
            ...
            noteList.querySelectorAll("button[data-nid]").forEach((btn) => {
              btn.onclick = async () => {
                ...
                try {
                  await api2(`/workspace/notes/${btn.dataset.nid}`, { method: "DELETE" });
                  renderWsNotes();
                } catch (e) {
                  alert("Error: " + e);
                }
        ...
        if (noteAdd) noteAdd.onclick = async () => {
          ...
          try {
            await api2("/workspace/notes", { method: "POST", body: JSON.stringify({ title, body: body2 }) });
      ...
      <div id="invite-status-${projectId}" style="font-size:10px;color:var(--muted);margin-top:5px;min-height:14px"></div>
        ...
        setTimeout(async () => {
          ...
          async function renderMembers() {
            if (!listEl) return;
            try {
              const members = await api2("/workspace/members");
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;padding:4px 0;border-bottom:1px solid var(--border)">
              ...
              listEl.querySelectorAll("select.member-role-select").forEach((sel) => {
                ...
                sel.onchange = async () => {
                  ...
                  sel.disabled = true;
                  try {
                    await api2(`/workspace/members/${sel.dataset.mid}`, {
                      method: "PATCH",
              ...
              listEl.querySelectorAll("button.resend-invite-btn").forEach((btn) => {
                btn.onclick = async () => {
                  ...
                  btn.textContent = "\u2026";
                  try {
                    await api2(`/workspace/invite/${btn.dataset.mid}/resend`, { method: "POST" });
              ...
              listEl.querySelectorAll("button[data-mid]:not(.resend-invite-btn)").forEach((btn) => {
                btn.onclick = async () => {
                  ...
                  try {
                    await api2(`/workspace/members/${btn.dataset.mid}`, { method: "DELETE" });
                    renderMembers();
                  } catch (e) {
                    alert("Error: " + e);
                  }
          ...
          if (inviteBtn) {
            inviteBtn.onclick = async () => {
              ...
              try {
                await api2("/workspace/invite", { method: "POST", body: JSON.stringify({ email, role }) });
      ...
      <div style="border:1px solid #7f1d1d;border-radius:4px;padding:10px;background:#1a0a0a">
        ...
        setTimeout(() => {
          ...
          deleteBtn2.onclick = async () => {
            ...
            try {
              await api2("/account/delete", { method: "POST", body: JSON.stringify({ confirmation: "DELETE" }) });
      ...
      <div id="usage-body-${projectId}" style="font-size:10px;color:var(--muted)">loading\u2026</div>
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            const u = await api2("/settings/usage");
            const c = u.compute || {};
            const s = u.storage || {};
          ...
          <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap">
            ...
            if (saveBtn) {
              saveBtn.onclick = async () => {
                ...
                saveBtn.disabled = true;
                try {
                  await api2("/settings/usage", { method: "PATCH", body: JSON.stringify({ compute_cap: cc, storage_cap: sc }) });
                  if (capsStatus) {
    ...
    <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
      ...
      <input type="text" id="ntfy-url-${projectId}"
        ...
        placeholder="${escapeHtml(suggestNtfyTopic(projectId))}  \xB7  https://hooks.slack.com/\u2026"
    ...
    <div style="font-size:9px;color:var(--muted);margin-top:4px;line-height:1.6">
      ...
      setTimeout(() => {
        ...
        const renderHooksTokenList = (tokens) => {
          ...
          tokenListEl.querySelectorAll("[data-token-id]").forEach((btn) => {
            btn.onclick = async () => {
              ...
              btn.disabled = true;
              try {
                await api2(`/auth/tokens/${tokenId}`, { method: "DELETE" });
                const statusEl = document.getElementById(`hooks-token-status-${projectId}`);
        ...
        async function loadHooksTokens() {
          ...
          try {
            const tokens = await api2("/auth/tokens");
        ...
        if (genBtn) {
          genBtn.onclick = async () => {
            ...
            genBtn.textContent = "Generating...";
            try {
              const tok = await api2("/auth/tokens", { method: "POST", body: JSON.stringify({ label: "hooks-config" }) });
        ...
        if (genReadonlyBtn) {
          genReadonlyBtn.onclick = async () => {
            ...
            try {
              const tok = await api2("/auth/tokens", { method: "POST", body: JSON.stringify({ label: "readonly", token_type: "readonly" }) });
      ...
      if (ntfySaveBtn) {
        ntfySaveBtn.onclick = async () => {
          ...
          try {
            const saved = await api2(`/projects/${projectId}/ntfy`, {
              method: "PATCH",
      ...
      if (notifyEmailSaveBtn) {
        notifyEmailSaveBtn.onclick = async () => {
          ...
          const raw = (inp ? inp.value : "").trim() || null;
          try {
            await api2(`/projects/${projectId}/ntfy`, {
              method: "PATCH",
      ...
      if (ntfyTestBtn) {
        ntfyTestBtn.onclick = async () => {
          ...
          try {
            await api2(`/projects/${projectId}/notify/test`, { method: "POST", body: "{}" });
            if (statusEl) {
      ...
      body.querySelectorAll("input[data-pref]").forEach((cb) => {
        cb.onchange = async () => {
          ...
          try {
            await api2("/settings/notifications", { method: "PATCH", body: JSON.stringify(payload) });
            if (statusEl) {
              statusEl.textContent = "saved";
              setTimeout(() => {
                statusEl.textContent = "";
      ...
      if (ghSaveBtn) {
        ghSaveBtn.onclick = async () => {
          ...
          if (statusEl) statusEl.textContent = "";
          try {
            await api2(`/projects/${projectId}/github/connect`, {
              method: "POST",
      ...
      if (ghDisconnectBtn) {
        ghDisconnectBtn.onclick = async () => {
          ...
          try {
            await api2(`/projects/${projectId}/github/disconnect`, { method: "DELETE" });
            loadSettingsTab(projectId);
          } catch (e) {
      ...
      if (ghBranchSelect) {
        const fillBranches = async (repo, preferred) => {
          ...
          try {
            const res = await api2(`/projects/${projectId}/github/branches?repo=${encodeURIComponent(repo || "")}`);
      ...
      if (ghTestBtn) {
        ghTestBtn.onclick = async () => {
          ...
          try {
            const st = await api2(`/projects/${projectId}/github/status`);
            if (statusEl) {
  ...
  async function loadNotesTab(projectId) {
    ...
    if (addBtn) addBtn.onclick = async () => {
      ...
      try {
        await api2(`/projects/${projectId}/notes`, {
          method: "POST",
  ...
  async function loadHitlTab(projectId) {
    ...
    const render = async () => {
      ...
      try {
        const rows = await api2(`/projects/${projectId}/hitl${qs}&limit=50`);
        ...
        body.querySelectorAll(".hitl-answer-btn").forEach((btn) => {
          btn.onclick = async () => {
            ...
            try {
              await api2(`/hitl/${id}`, { method: "PATCH", body: JSON.stringify({ action: "answer", answer }) });
              toast("answered \u2713");
              render();
            } catch (e) {
              toast("failed: " + e.message, true);
        ...
        body.querySelectorAll(".hitl-dismiss-btn").forEach((btn) => {
          btn.onclick = async () => {
            ...
            try {
              await api2(`/hitl/${btn.dataset.hitlId}`, { method: "PATCH", body: JSON.stringify({ action: "dismiss" }) });
              toast("dismissed");
              render();
            } catch (e) {
              toast("failed: " + e.message, true);
        ...
        body.querySelectorAll(".hitl-approve-btn").forEach((btn) => {
          btn.onclick = async () => {
            ...
            try {
              const res = await api2(`/hitl/${btn.dataset.hitlId}`, { method: "PATCH", body: JSON.stringify({ action: "answer", answer: "approved" }) });
  ...
  async function loadTeamTab(projectId) {
    ...
    const render = async () => {
      ...
      try {
        ...
        let decisionsHtml = "";
        try {
          const pinned = await api2(`/projects/${projectId}/decisions-pinned`);
  ...
  async function updateLiveFeed(projectId) {
    ...
    const panel = getPanelState(projectId);
    try {
      const sessions = await api2(`/projects/${projectId}/sessions?active_only=true`);
      ...
      const tasks = await api2(`/projects/${projectId}/sessions/${sess.id}/tasks/live?limit=5`).catch(() => []);
  ...
  async function loadRecentSessions(projectId, sessions = null) {
    ...
    try {
      const panel = getPanelState(projectId);
      const allSessions = Array.isArray(sessions) ? sessions : await api2(`/projects/${projectId}/sessions?active_only=false`);
  ...
  async function loadMilestones(projectId) {
    ...
    if (!el) return;
    try {
      const all = await api2(`/projects/${projectId}/sprint-items`);
  ...
  async function loadRecentRuns(projectId) {
    ...
    try {
      const runs = await api2(`/projects/${projectId}/runs?limit=10`);
      if (!runs || !runs.length) {
      ...
      body.querySelectorAll(".run-row").forEach((row) => {
        row.addEventListener("click", async () => {
          ...
          if (!transcript.textContent.trim()) {
            try {
              const full = await api2(`/projects/${pid}/runs/${runId}`);
        ...
        if (!searchInput._wired) {
          ...
          searchInput.addEventListener("input", function() {
            ...
            _searchTimer = setTimeout(async () => {
              ...
              try {
                const results = await api2(`/projects/${projectId}/search?q=${encodeURIComponent(q)}&limit=10`);
  ...
  window._queueAction = async function(taskId, action) {
    try {
      if (action === "delete") {
        ...
        await api2(`/tasks/${taskId}`, { method: "DELETE" });
      } else if (action === "done") {
        await api2(`/tasks/${taskId}`, { method: "PATCH", body: JSON.stringify({ status: "done" }) });
      } else if (action === "backlog") {
        await api2(`/tasks/${taskId}`, { method: "PATCH", body: JSON.stringify({ status: "backlog" }) });
      }
  ...
  async function loadFilesTab(projectId) {
    ...
    if (!listEl) return;
    try {
      const files = await api2(`/projects/${projectId}/files`);
  ...
  async function openFileEditor(projectId, filename) {
    ...
    try {
      const data = await api2(`/projects/${projectId}/files/${encodeURIComponent(filename)}`);
  ...
  async function saveFile(projectId) {
    ...
    if (!filename) return;
    try {
      await api2(`/projects/${projectId}/files/${encodeURIComponent(filename)}`, {
        method: "PUT",
  ...
  function renderConstitutionWarning(projectId) {
    ...
    if (archiveBtn) {
      archiveBtn.onclick = async () => {
        ...
        try {
          await api2(`/projects/${projectId}/decisions-pinned/archive-oldest`, {
            method: "POST",
  ...
  async function refreshProjectCountBadges(projectId) {
    ...
    const [notesRes, pinnedRes] = await Promise.allSettled([
      ...
      api2(`/projects/${projectId}/decisions-pinned`)
    ]);
  ...
  async function refreshHitl() {
    ...
    try {
      const items = await api2("/hitl?status=pending&limit=50");
    ...
    try {
        ...
        tag.onclick = (e) => {
          ...
          sel.onchange = async () => {
            ...
            try {
              ...
              await api2(`/projects/${pid}/decisions-pinned/${id}`, { method: "PATCH", body: JSON.stringify({ category: newCat }) });
      ...
      host.querySelectorAll("[data-archive-decision]").forEach((btn) => {
        btn.onclick = async () => {
          ...
          try {
            await api2(`/projects/${projectId}/decisions-pinned/${id}`, {
              method: "PATCH",
      ...
      if (toggleEl) {
        if (showArchived) {
          ...
          api2(`/projects/${projectId}/decisions-pinned?include_superseded=true`).then((all) => {
      ...
      try {
        const all = await api2(`/projects/${projectId}/decisions-pinned?include_superseded=true`);
  ...
  async function supersedePinnedDecision(projectId, decisionId) {
    ...
    if (!newBody) return;
    try {
      await api2(`/projects/${projectId}/decisions-pinned/${decisionId}`, {
        method: "PATCH",
  ...
  async function addPinnedDecision(projectId) {
    ...
    if (!category) return;
    try {
      await api2(`/projects/${projectId}/decisions-pinned`, {
        method: "POST",
  ...
  async function consolidateDecisions(projectId) {
    ...
    document.getElementById("_consolidate-run").onclick = async () => {
      ...
      try {
        const result = await api2(`/projects/${projectId}/decisions/consolidate`, {
          method: "POST",
        ...
        document.getElementById("_preview-apply").onclick = async () => {
          ...
          try {
            await api2(`/projects/${projectId}/decisions-pinned/replace-all`, {
              method: "POST",
  ...
  async function saveNorthStar(projectId) {
    ...
    try {
      ...
      await api2(`/projects/${projectId}/goal/north-star`, {
        method: "POST",
  ...
  async function saveSprint(projectId) {
    ...
    if (!val) return;
    try {
      await api2(`/projects/${projectId}/goal/sprint`, {
        method: "POST",
  ...
  async function _loadMoreTasks(projectId, btn) {
    ...
    try {
      const more = await api2(`/projects/${projectId}/tasks?limit=100&offset=${offset}`);
  ...
  async function appendToGoal(projectId, line) {
    let current = "";
    try {
      const goal = await api2(`/projects/${projectId}/goal`);
      current = typeof goal.content === "string" ? goal.content : JSON.stringify(goal.content, null, 2);
    } catch (e) {
    }
    const next = current ? current.trimEnd() + "\n" + line : line;
    await api2(`/projects/${projectId}/goal`, { method: "POST", body: JSON.stringify({ content: next }) });
  }
  async function hitlReply(projectId, taskId, text) {
    try {
      await appendToGoal(projectId, `[HITL-REPLY:${taskId}:] ${text}`);
      await api2(`/tasks/${taskId}`, { method: "PATCH", body: JSON.stringify({ status: "done", description: `[ANSWERED] ${text}` }) });
      toast("reply sent");
    } catch (e) {
  ...
  async function hitlExecute(projectId, taskId, confirmed) {
    try {
      if (confirmed) {
        ...
        await api2(`/tasks/${taskId}`, { method: "PATCH", body: JSON.stringify({ status: "failed" }) });
  ...
  document.getElementById("new-project-btn").onclick = async () => {
    ...
    try {
      const p = await api2("/projects", { method: "POST", body: JSON.stringify(body) });
      inp.value = "";
  ...
  async function restoreTabs() {
    ...
    let preferred = null;
    try {
      preferred = localStorage.getItem(STORAGE_KEY(ACTIVE_PROJECT_KEY));
    } catch (e) {
    }
    try {
      saved = JSON.parse(localStorage.getItem(STORAGE_KEY(TABS_KEY)) || "[]");
    } catch (e) {
    }
  ...
  (async function init() {
    ...
    if (!isHostedMode()) {
      ...
      if (stopBtn) {
        stopBtn.onclick = async () => {
          ...
          try {
            await api2("/admin/shutdown", { method: "POST" });
    ...
    async function _checkGitStatus() {
      try {
        const data = await api2("/admin/git-status");
  ...
  async function _deleteSprintItem(projectId, itemId) {
    ...
    try {
      await api2(`/projects/${projectId}/sprint-items/${itemId}`, { method: "DELETE" });
  ...
  async function _sprintAction(projectId, itemId, action) {
    try {
      await api2(`/projects/${projectId}/sprint-items/${itemId}/${action}`, { method: "POST" });
      if (_sprintBoardReloaders[projectId]) _sprintBoardReloaders[projectId]();
    } catch (e) {
  ...
  async function completeSprintItem(projectId, itemId) {
    try {
      await api2(`/projects/${projectId}/sprint-items/${itemId}/complete`, { method: "POST" });
      if (_sprintBoardReloaders[projectId]) _sprintBoardReloaders[projectId]();
    } catch (e) {
  ...
  async function failSprintItem(projectId, itemId) {
    try {
      await api2(`/projects/${projectId}/sprint-items/${itemId}/fail`, { method: "POST" });
      if (_sprintBoardReloaders[projectId]) _sprintBoardReloaders[projectId]();
    } catch (e) {
    ...
    if (searchInp && !searchInp._wired) {
      ...
      searchInp.addEventListener("input", function() {
        ...
        _st = setTimeout(async () => {
          ...
          try {
            const results = await api2(`/projects/${projectId}/search?q=${encodeURIComponent(q)}&limit=15`);
  ...
  async function loadRewindTab(projectId, days) {
    ...
    try {
      const [data, history, stats] = await Promise.all([
        api2(`/projects/${projectId}/rewind?days=${days}`),
        api2(`/projects/${projectId}/goal-history`).catch(() => []),
        api2(`/projects/${projectId}/stats?days=30`).catch(() => null)
      ]);
