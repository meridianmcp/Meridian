...
export async function loadSettingsTab(projectId: any, { force = false } = {}) {
  ...
  setTimeout(() => {
    ...
    saveBtn.onclick = async () => {
      ...
      // 3248f35d — auto-prospecting toggle. Persist explicit true/false so the
      // backend default (True when absent) can be turned OFF and stay off.
      const enrichEl = document.getElementById(`exec-enrich_prospect-${projectId}`) as HTMLInputElement | null;
      ...
      if (inviteBtn) {
        ...
        inviteBtn.onclick = async () => {
          ...
          try {
            ...
            await api('/workspace/invite', { method: 'POST', body: JSON.stringify({ email, role, project_id: scopedProjectId || undefined }) });
  ...
  // ── BLOG CMS group (admin only, 6234f9b8) ───────────────────────────────
  // Hosted: admin plan only. Self-host: always (single-user, API allows freely).
  if ((window.state.tenantPlan || '') === 'admin' || !isHostedMode()) {
    ...
    html += `<details id="settings-grp-blog-${projectId}" style="margin-bottom:12px;border:2px solid var(--border);border-radius:8px">` +
      `<summary style="cursor:pointer;list-style:none;padding:10px 14px;display:flex;align-items:center;gap:8px;background:var(--surface-2);border-radius:8px">` +
      `<span style="font-weight:700;font-size:11px;color:var(--text);letter-spacing:.04em">BLOG <span style="color:var(--muted)">(admin)</span></span></summary>` +
      `<div style="padding:10px">` +
      `<div style="font-size:10px;color:var(--muted);margin-bottom:8px">Authoring is admin-only. Publish makes a post live at <code>/blog/&lt;slug&gt;</code> immediately.</div>` +
      `<input id="blog-edit-id" type="hidden">` +
      `<input id="blog-title" type="text" placeholder="Post title" style="${_inp};margin-bottom:6px">` +
      `<textarea id="blog-body" rows="8" placeholder="# Heading&#10;&#10;Markdown body. **bold**, \`code\`, fenced blocks." style="${_inp};resize:vertical"></textarea>` +
      `<div style="display:flex;gap:6px;margin-top:6px;align-items:center;flex-wrap:wrap">` +
      `<button id="blog-save" class="primary" style="font-size:10px;padding:3px 10px">Save draft</button>` +
      `<button id="blog-gen" class="secondary" style="font-size:10px;padding:3px 10px">Generate from activity</button>` +
      `<button id="blog-new" class="secondary" style="font-size:10px;padding:3px 10px">New</button>` +
      `<span id="blog-status" style="font-size:10px;color:var(--muted)"></span></div>` +
      `<div id="blog-list" style="margin-top:12px;font-size:11px"><div style="color:var(--muted)">loading…</div></div>` +
      `</div></details>`;
  }
      ...
      try {
        ...
        listEl.innerHTML = posts.map((p: any) => {
          ...
          return `<div style="display:flex;align-items:center;gap:6px;padding:5px 0;border-bottom:1px solid var(--border)">
            ...
            <button class="secondary blog-del" data-id="${escapeHtml(p.id)}" style="font-size:9px;padding:2px 7px">🗑</button>
          </div>`;
        }).join('');
  ...
  setTimeout(() => {
    ...
    // Manual path entry: GET current settings, merge {cwd, hostname} into
    // executor_config.repo_paths (deduped), PATCH back — so a hand-added path
    // persists immediately and coexists with hook-registered entries.
    ...
    const _doAddPath = async () => {
      ...
      if (!cwd) { if (ezStatus) ezStatus.textContent = 'Enter a cwd path.'; return; }
      ...
      try {
        ...
        await saveProjectSettings(projectId, { executor_config: cfg });
        _ezPaths = paths;            // keep in-memory state in sync with persisted
        _rerenderPathsTbl();
    ...
    if (_ezAddCwd) _ezAddCwd.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); _doAddPath(); } });
