...
_MCP_TOOLS_LIST: list[dict[str, Any]] = [
    ...
    {"name": "batch_read", "description":
        ...
        "'get_profile_layer_revisions' (args: scope_id, optional limit); and 'tunnel_research' "
        "(d17a437a, bounded cross-MCP research fan-out) with operations 'diagnostics' (args: "
        "optional refresh bool — reports which of the code/docs/outputs tunnel slots are "
        "connected right now and which specific tool names are routable on them; never fails on "
        "a missing tunnel context) and 'call' (args: tool, optional arguments object — dispatches "
        "ONE READ-ONLY tool call through whichever connected tunnel slot serves it; 'tool' must be "
        "on this adapter's own fixed read-only allowlist for the code-intel/meridian-docs/"
        "meridian-outputs MCP surfaces or it is rejected as VALIDATION_ERROR before any dispatch is "
        "attempted; NOT_FOUND when no tunnel is active or the tool isn't exposed on any connected "
        "slot — never dispatched blind). Returns {results: [{request_id, status, adapter, operation, result, "
    ...
    {"name": "get_tunnel_diagnostics", "description":
        ...
        "43fcdf9f — HONESTY CONTRACT: healthy_reported/state are a CACHED "
        "last-known-state flag from the last plugin_status message the server "
        "received (or the default when none ever arrived), NEVER a live, "
        "request-level probe of that slot right now — a slot can read "
        "'healthy' here while real calls through it are actively failing "
        "(e.g. an edge/Cloudflare error the client never self-reports). Use "
        "health_basis ('client_reported' vs 'default_assumed'), "
        "health_reported_at, and health_age_seconds on each slot to see how "
        "stale that flag actually is before trusting a green status. "
